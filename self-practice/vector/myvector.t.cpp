#include "myvector.h" // Your vector header file
...
int VectorTest::TestObject::constructions = 0;
int VectorTest::TestObject::destructions = 0;

// Reset counters before each test
void ResetTestObjectCounters() {
  VectorTest::TestObject::constructions = 0;
...
TEST_F(VectorTest, SeparateAllocationAndConstruction) {
  ResetTestObjectCounters();
  {
    ...
    EXPECT_EQ(TestObject::constructions, 0)
        << "No objects should be constructed during reserve";
    vec.emplace_back(42); // Construct one object
    EXPECT_EQ(TestObject::constructions, 1)
        << "One object should be constructed";
    EXPECT_EQ(TestObject::destructions, 0)
        << "No objects should be destroyed yet";
  }
  EXPECT_EQ(TestObject::destructions, 1)
      << "Object should be destroyed on vector destruction";
}

TEST_F(VectorTest, SeparateDeallocationAndDestruction) {
  ResetTestObjectCounters();
  {
    Vector<TestObject> vec;
    vec.reserve(10);
    ...
    EXPECT_EQ(TestObject::destructions, 2)
        << "Objects should be destroyed on clear";
    EXPECT_EQ(TestObject::constructions, 2)
        << "No new constructions during clear";
  }
  EXPECT_EQ(TestObject::destructions, 2)
      << "No additional destructions after clear";
}

TEST_F(VectorTest, ResizingMechanism) {
  ResetTestObjectCounters();
  {
    Vector<TestObject> vec;
...
TEST_F(VectorTest, ConstIteratorTraversal) {
  Vector<int> vec;
  vec.push_back(1);
  vec.push_back(2);
  vec.push_back(3);
  ...
  for (auto it = const_vec.begin(); it != const_vec.end(); ++it) {
    EXPECT_EQ(*it, expected[i++])
        << "Const iterator should traverse elements in order";
  }
  EXPECT_EQ(i, 3);
}

TEST_F(VectorTest, ReverseIteratorTraversal) {
  Vector<int> vec;
  vec.push_back(1);
  vec.push_back(2);
  vec.push_back(3);
  ...
  for (auto it = vec.rbegin(); it != vec.rend(); ++it) {
    EXPECT_EQ(*it, expected[i++])
        << "Reverse iterator should traverse elements in reverse";
  }
  EXPECT_EQ(i, 3);
}

TEST_F(VectorTest, IteratorModification) {
  Vector<int> vec;
  vec.push_back(1);
  vec.push_back(2);
  vec.push_back(3);
